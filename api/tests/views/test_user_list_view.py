...
class UserListViewTest(ListViewTestCase):
  def test_should_get_succeed(self):
    self.should_get_all_succeed(USER_LIST_URL, User, token=self.admin_user.token)

  def test_should_get_require_role(self):
    response = self.client.get(USER_LIST_URL, HTTP_AUTHORIZATION='Bearer {0}'.format(self.reader_user.token))
  ...
  def test_should_post_null_values(self):
    ...
    self.should_post_fail(USER_LIST_URL, data=data, expected_content=expected_content, token=self.admin_user.token)
