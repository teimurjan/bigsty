...
class FeatureValueViewTest(DetailViewTestCase):
  ...
  def test_should_delete_succeed(self):
    feature_value = FeatureValue.objects.all()[0]
    self.should_delete_succeed(FEATURE_VALUE_LIST_URL, feature_value.pk, self.manager_user_token)

  def test_should_delete_require_role(self):
    feature_value = FeatureValue.objects.all()[0]
    self.should_delete_require_role(FEATURE_VALUE_LIST_URL, feature_value.pk, self.reader_user_token)

  def test_should_delete_not_found(self):
    self.should_delete_not_found(FEATURE_VALUE_LIST_URL, FeatureValue, 999, self.manager_user_token)
